...
export async function invokeGoogleProvider(request: ManagerProviderRequest): Promise<ManagerProviderResult> {
  ...
  if (!response.ok) {
    const errorMessage = String(json?.error?.message || json?.error || `Google HTTP ${response.status}`);
    throw new Error(errorMessage);
  }

  const text = extractChatText(json);
  if (!text) throw new Error("Google returned an empty manager response.");
