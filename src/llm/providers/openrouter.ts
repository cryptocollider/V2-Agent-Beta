...
export async function invokeOpenRouterProvider(request: ManagerProviderRequest): Promise<ManagerProviderResult> {
  ...
  if (!response.ok) {
    const errorMessage = String(json?.error?.message || json?.error || `OpenRouter HTTP ${response.status}`);
    throw new Error(errorMessage);
  }

  const text = extractChatText(json);
  if (!text) throw new Error("OpenRouter returned an empty manager response.");
